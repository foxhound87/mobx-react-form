...
export const checkObserve = collection => change => collection.map(checkObserveItem(change));
...
export const hasProps = ($type, $data) => {
  ...
  switch ($type) {
    case 'booleans':
      ...
      break;
    case 'field':
      $props = [...props.field, ...props.validation, ...props.function, ...props.handlers];
      break;
    case 'all':
      $props = [
        'id',
...
export const isPromise = obj =>
  !!obj && typeof obj.then === 'function' && (typeof obj === 'object' || typeof obj === 'function');

export const isStruct = ({ fields }) => _.isArray(fields) && _.every(fields, _.isString);

export const isEmptyArray = field => _.isEmpty(field) && _.isArray(field);

export const isArrayOfObjects = fields => _.isArray(fields) && _.every(fields, _.isPlainObject);

export const $getKeys = fields => _.union(_.map(_.values(fields), values => _.keys(values))[0]);
...
export const allowNested = (field, strictProps) =>
  ...
  !_.isDate(field) &&
  !_.has(field, 'fields') &&
  (!hasSome(field, [...props.field, ...props.validation, ...props.function, ...props.handlers]) ||
    strictProps);

export const parseIntKeys = fields => _.map(fields.keys(), _.ary(parseInt, 1));
...
export const $isBool = ($, val) => _.isBoolean(val) && _.isBoolean($.target.checked);
