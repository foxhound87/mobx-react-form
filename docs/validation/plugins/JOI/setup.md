...
> **Note:** JOI does not support the `extend` callback or async validation pipelines through mobx-formkit's plugin system. For custom validation and async rules, use JOI's native `.custom()` method on the schema directly.
