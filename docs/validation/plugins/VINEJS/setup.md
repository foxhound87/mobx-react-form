...
VineJS `validate()` is async-by-design at the API level — even for synchronous rules. The driver consumes the mobx-react-form async contract (`setValidationAsyncData` + `promises`), so always await validation:
