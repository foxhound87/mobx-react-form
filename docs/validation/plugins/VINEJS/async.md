...
VineJS `validate()` is **async-by-design** at the API level — even for synchronous rules. The **VINEJS** driver consumes the mobx-react-form async contract (`setValidationAsyncData` + `promises`), so all validation flows through the promise pipeline.
