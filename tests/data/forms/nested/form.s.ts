      ...
      call: ({ form, change }) => {
        describe("Check Nested-S $value@club.city Disposers:", () =>
          it("Disposers should have $value@club.city prop", () =>
            expect(form.state.disposers.interceptor).to.have.property(
              "$value@club.city"
            )));
  ...
  club: [
    {
      key: "value",
      call: ({ form }) =>
        describe("Check Nested-S value@club Disposers:", () =>
  ...
  members: [
    {
      key: "fields",
      call: ({ form }) =>
        describe("Check Nested-S fields@members Disposers:", () =>
  ...
  hooks() {
    return {
      onInit(form: FormInterface) {
        this.$("club.name").intercept((data: any) => {
